...
const tones: Record<BadgeTone, string> = {
  ...
  neutral: 'bg-tc-surface border-tc-border text-tc-text',
...
export function Badge({ tone = 'neutral', className = '', children, ...rest }: Props) {
  return (
    <span {...rest} className={`inline-flex items-center gap-1 rounded-md border px-2 py-0.5 text-xs font-semibold ${tones[tone]} ${className}`}>
      {children}
    </span>
  )
}
...
export function Pill({ tone = 'neutral', className = '', children, ...rest }: Props) {
  return (
    <span {...rest} className={`inline-flex items-center rounded-md border px-2.5 py-1 text-xs font-semibold ${tones[tone]} ${className}`}>
      {children}
    </span>
  )
}
