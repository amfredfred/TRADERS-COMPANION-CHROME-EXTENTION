...
export default function App() {
  ...
  return (
    <div className="flex h-screen flex-col bg-tc-bg text-tc-text" style={{ fontFamily: 'Inter, ui-sans-serif, system-ui, sans-serif' }}>
      ...
      <main className="tc-scrollbar min-h-0 flex-1 overflow-y-auto p-4">
        {activeTab === 'dashboard' && (
          <DashboardTab
            attached={attached}
            tabStatus={tabStatus}
...
function SidecarHeader({ statusLabel: status, tone, domain, onCollapse, onUnpin }: { statusLabel: string; tone: 'success' | 'warning' | 'neutral'; domain: string; onCollapse: () => void; onUnpin: () => void }) {
  return (
    <header className="flex h-[60px] shrink-0 items-center justify-between gap-3 border-b border-tc-border/50 bg-tc-panel px-4">
      ...
      <div className="flex shrink-0 items-center gap-2">
        ...
        <Button size="sm" variant="ghost" onClick={onCollapse}>Collapse</Button>
  ...
  return (
    <div className="space-y-4">
      ...
      <Card padding="sm">
        ...
        <StatRow label="Trades today" value={session ? `${session.tradesOpenedToday} / ${session.maxTrades}` : settings ? `Manual session required` : 'Not available'} />
...
function ChatTab({ tabStatus, messages, activities, busy, input, bottomRef, onInput, onCapture, onSubmit, onPrompt }: {
  tabStatus: CurrentTabStatusResponse | null
    ...
    <div className="flex min-h-full flex-col">
      <div className="mb-4 flex items-start justify-between gap-3">
        <div>
          ...
          <div className="mt-1 text-xs text-tc-muted">{tabStatus?.snapshot?.platformName ?? tabStatus?.domain ?? 'Current tab'}</div>
        </div>
      ...
      <div className="sticky bottom-0 mt-5 border-t border-tc-border/50 bg-tc-bg pt-3">
...
function SessionTab({ session, settings, manualBalance, manualTradeOpen, onManualBalance, onStartManual, onNoTradeMode, onReset, onManualTradeOpen }: {
  ...
  return (
    <div className="space-y-4">
      ...
      <Card padding="sm">
        <StatRow label="Session lock" value={session ? `Locked at ${new Date(session.startedAt).toLocaleTimeString([], { hour: '2-digit', minute: '2-digit' })}` : 'No session'} />
        ...
        <StatRow label="Last trade result" value={session?.lastTradeClosedAt ? 'Closed today' : 'Not available'} />
